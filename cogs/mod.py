...
class mod():
    ...
    async def kick(self, ctx, user: discord.Member=None, *reason):
        for perms in ctx.message.author.server_permissions:
            if perms == discord.Permissions.kick_members:
                if user is None:
                    embed = discord.Embed(title="Error!", description="Please specify a user to kick", color=0xffbc77)
    ...
    async def ban(self, ctx, user: discord.Member=None, *reason):
        for perms in ctx.message.author.server_permissions:
            if perms == discord.Permissions.ban_members:
                if user is None:
                    embed = discord.Embed(title="Error!", description="Please specify a user to ban", color=0xffbc77)
                    await self.bot.say(embed=embed)
                else:
                    reason = ' '.join(reason)
                    if reason == "":
                    ...
                    try:
                        ...
                        pass
                    dm = discord.Embed(title="VAC banned from secure server", description="", color=0xffbc77)
                    dm.add_field(name="Server name", value=ctx.message.server.name)
                    dm.add_field(name="Server id", value=ctx.message.server.id, inline=True)
                    dm.add_field(name="Reason", value=reason)
